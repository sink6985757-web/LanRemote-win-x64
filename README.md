...
10. `File > 檔案傳輸` 或 Toolbar「檔案傳輸」可開啟雙向路徑視窗；在本機檔案總管複製檔案後，對遠端畫面按 `Ctrl+V` 也會進入檔案傳輸。
11. `View > 狀態資訊` 可選擇關閉、簡易（預設）與詳細；詳細模式會多一條薄診斷列。
12. 工具列隱藏後，把滑鼠移到視窗最頂端即可重新顯示。
13. 反向控制時先斷線，再讓兩臺交換 A／B 角色並重新配對；需使用 SAS 的兩端都要各自手動安裝服務。
...
- ReadyGate（`WO-LANREMOTE-V4-FILE-TRANSFER-UX-20260823`）：協定與本機實作驗證進行中；正式發布仍停止，等待 v4 Windows 10／11 雙向檔案拖放、視窗、剪貼簿、續傳與原有控制功能實機證據。
