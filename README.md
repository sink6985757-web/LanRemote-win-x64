...
目前提供未簽章的本機測試候選，不是公開發布版本。協定 v2 已在 Windows 11 25H2 以兩個真實程式實例完成 TLS 配對、畫面傳送、介面與畫質切換驗證；Windows 10 22H2 與 Windows 11 25H2 的新版雙機驗證仍待使用者執行。
...
- 視窗化、最大化與 F11 無邊框全螢幕；視窗化影像等比例自適應並正確處理黑邊座標。
...
- 協定 v2 具有 magic、版本、訊息類型、payload 長度上限、nonce 驗證與畫質套用回覆。
- 僅接受 loopback、RFC1918 或 IPv4 link-local 位址。

## 目前限制

- v2 不向下相容；兩臺都必須使用同一份新版測試包。
...
6. 連線後從頂部工具列切換視窗模式或畫質；F11 切換全螢幕，Esc 退出全螢幕。
7. 工具列隱藏後，把滑鼠移到視窗最頂端即可重新顯示。
8. 反向控制時先斷線，再讓兩臺交換 A／B 角色並重新配對。
...
| 路徑 | 責任 |
|---|---|
| `src/LanRemote.Protocol` | v2 framing、JSON payload、畫面、輸入與畫質訊息 |
| `src/LanRemote.Core` | TLS session、SAS 配對、私人 IP 政策、host/controller 與影像座標映射 |
| `src/LanRemote.Windows` | 主要螢幕 JPEG 擷取、即時畫質設定與受限 `SendInput` 注入 |
| `src/LanRemote.App` | WPF 單視窗 launcher/session、傳統選單、工具列與遠端輸入 |
...
- ReadyGate（`WO-LANREMOTE-UX-PROFILES-V1`）：`CONDITIONAL` 本機測試候選；停止正式發布，等待 v2 Windows 10／11 雙向實機證據。
