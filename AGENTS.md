...
4. MVP 不包含網際網路 relay、無人值守存取、UAC／安全桌面繞過、提權、文字／圖片剪貼簿或音訊；檔案傳輸只限已配對且接收端明確開啟的當次 session。
...
9. 線上協定目前為 v4；同一 session 兩端必須使用相同主要版本，不得對舊版靜默降級。
...
13. 檔案傳輸每次斷線即失效；不得安靜收檔、覆寫同名檔、寫入 Windows／Program Files／Startup、追隨 reparse point／symlink／junction、接受 ADS／UNC／路徑逸出，或提供遠端刪除／執行功能。
14. 僅在使用者明確拖放、貼上檔案或點選 Toolbar 按鈕時讀取檔案清單；不得持續監控剪貼簿。
